...
class TestImageAnalysis(unittest.TestCase):
    ...
    def test_pixel_processing(self):
        ...
        np.testing.assert_almost_equal(acaana.pixel_processing(data),result_processing)


    def test_optimal_w(self):
        random = np.random.RandomState(0)
        data = random.rand(100,100)
        np.testing.assert_almost_equal(acaana.optimal_w(data, p=0.3),25)

    # This was moved to Algorithms (and is candidate for deprecation!)
...
# This was moved to transform, but is in the path for deprecation (nobody use it!)
